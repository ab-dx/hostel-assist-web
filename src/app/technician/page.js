...
import { useTechnicianTickets } from "@/lib/useTechnicianTickets";
import TicketCardTech from "@/components/ticket-card-tech";

export default function Technician() {
  const { authUser, loading, signInWithGoogle, signOutUser } = useAuth();
  const router = useRouter();
  const { tickets, loading: loadingTickets } = useTechnicianTickets();
              ...
              <TabsContent value="active">
                {loadingTickets ? "Active Tickets will be shown here" :
                  (tickets.filter(ticket => ticket.active).length > 0) ?
                    (tickets.filter(ticket => ticket.active).map(ticket => <TicketCardTech key={ticket.createdAt} {...ticket} ticketId={ticket.id} />))
                    : ("No active tickets")
                }
              </TabsContent>
